...
AUP_MODULE_AIVAD::AUP_MODULE_AIVAD(const char* onnx_path) {
...
int AUP_Aed_memAllocate(void* stPtr, const Aed_StaticCfg* pCfg) {
  return AUP_Aed_memAllocateWithModel(stPtr, pCfg, "onnx_model/ten-vad.onnx");
}

int AUP_Aed_memAllocateWithModel(void* stPtr, const Aed_StaticCfg* pCfg, const char* onnx_model_path) {
  Aed_St* stHdl = (Aed_St*)(stPtr);
  ...
  if (stHdl->aivadInf == NULL) {
    const char* resolved_model_path =
        (onnx_model_path != NULL && onnx_model_path[0] != '\0')
            ? onnx_model_path
            : "onnx_model/ten-vad.onnx";
    stHdl->aivadInf = new AUP_MODULE_AIVAD(resolved_model_path);
    if (stHdl->aivadInf == NULL) {
      return -1;
    }
  }
